...
function Menu:destroy()
  ...
  if not self.is_being_replaced then
    Elements.curtain:unregister("menu")
  end
...
function Menu:update(data)
  ...
  while i < #menus_to_serialize do
    ...
    if old_menu then
      table_assign(menu, old_menu, { "selected_index", "scroll_y", "fling" })
    end
...
function Menu:update_content_dimensions()
  ...
  for _, menu in ipairs(self.all) do
    ...
    if menu_title_width > max_width then
      max_width = menu_title_width
    end
...
function Menu:get_item_index_below_cursor()
  local menu = self.current
  if #menu.items < 1 or self.proximity_raw > 0 then
    return nil
  end
...
function Menu:open_selected_item_preselect()
  self:open_selected_item({ preselect_submenu_item = true })
end
function Menu:select_item_below_cursor()
  self.current.selected_index = self:get_item_index_below_cursor()
end
...
function Menu:on_global_mbtn_left_down()
  if self.proximity_raw == 0 then
    ...
    if cursor.x < self.ax and self.current.parent_menu then
      self:back()
    else
      self:close()
    end
...
function Menu:fling_distance()
  ...
  if mp.get_time() - last.time > 0.05 then
    return 0
  end
  for i = #self.drag_data - 1, 1, -1 do
    local drag = self.drag_data[i]
    if last.time - drag.time > 0.03 then
      return ((drag.y - last.y) / ((last.time - drag.time) / 0.03)) * 10
    end
  end
  ...
  if self.is_dragging then
    ...
    if math.abs(distance) > 50 then
      self.current.fling = {
        ...
        distance = distance,
        time = self.drag_data[#self.drag_data].time,
...
function Menu:on_global_mouse_move()
  ...
  if self.proximity_raw == 0 or self.is_dragging then
    self:select_item_below_cursor()
  else
    self.current.selected_index = nil
  end
...
function Menu:on_pgup()
  ...
  if menu.selected_index > 0 then
    self:scroll_to_index(menu.selected_index)
  end
end
...
function Menu:on_home()
  ...
  if self.current.selected_index > 0 then
    self:scroll_to_index(self.current.selected_index)
  end
end
...
function Menu:enable_key_bindings()
  ...
  self:add_key_binding("k", "menu-prev1", self:create_key_action("prev"), "repeatable")
  self:add_key_binding("j", "menu-next1", self:create_key_action("next"), "repeatable")
  self:add_key_binding("h", "menu-back1", self:create_key_action("back"))
  self:add_key_binding("l", "menu-select1", self:create_key_action("open_selected_item_preselect"))
  ...
  function draw_menu(menu, x, y, opacity)
    ...
    for index = start_index, end_index, 1 do
      ...
      if is_highlighted then
        separator_ay = item_by + 1
      end
      if next_is_highlighted then
        separator_by = item_by
      end
      if separator_by - separator_ay > 0 and item_by < by then
        ass:rect(ax + spacing / 2, separator_ay, bx - spacing / 2, separator_by, {
          color = fg,
          opacity = opacity * (item.separator and 0.08 or 0.06),
        })
      end
      ...
      if highlight_opacity > 0 then
        ass:rect(ax + 2, item_ay, bx - 2, item_by, {
          radius = 2,
          color = fg,
          opacity = highlight_opacity * text_opacity,
          clip = item_clip,
        })
      end
      ...
      if item.hint_width > 0 then
        ...
        title_cut_x = round(
          content_ax
            + (content_bx - content_ax - spacing) * title_content_ratio
      ...
      if item.title then
        ...
        ass:txt(content_ax, item_center_y, 4, item.ass_safe_title, {
          ...
          bold = item.bold,
          wrap = 2,
          opacity = text_opacity * (item.muted and 0.5 or 1),
          clip = clip,
          shadow = 1,
          shadow_color = shadow_color,
        })
