...
function Timeline:get_is_hovered()
  return self.enabled and (self.proximity_raw == 0 or self.hovered_chapter ~= nil)
end
...
function Timeline:update_dimensions()
  ...
  if Elements.top_bar.enabled then
    available_space = available_space - Elements.top_bar.size
  end
  ...
  if self.hovered_chapter then
    if not self.on_global_mbtn_left_down then
      self.on_global_mbtn_left_down = function()
        if self.hovered_chapter then
          mp.commandv("seek", self.hovered_chapter.time, "absolute+exact")
        end
      end
    end
  else
...
function Timeline:on_mouse_leave()
  if not self.hovered_chapter then
    self:clear_thumbnail()
  end
end
...
Timeline.seek_timer = mp.add_timeout(0.05, function()
  Elements.timeline:set_from_cursor()
end)
...
function Timeline:on_wheel_up()
  mp.commandv("seek", options.timeline_step)
end
function Timeline:on_wheel_down()
  mp.commandv("seek", -options.timeline_step)
end

function Timeline:render()
  if self.size_max == 0 then
    return
  end
  ...
  -- Chapters
  self.hovered_chapter = nil
  if options.timeline_chapters_opacity > 0 and (#state.chapters > 0 or state.ab_loop_a or state.ab_loop_b) then
  ...
  if
    (self.proximity_raw == 0 or self.pressed or self.hovered_chapter)
    and not (Elements.speed and Elements.speed.dragging)
  then
    ...
    if not thumbnail.disabled and thumbnail.width ~= 0 and thumbnail.height ~= 0 then
      ...
      local thumb_x = round(
        clamp(
          thumb_x_margin,
          cursor_x * scale_x - thumb_width / 2,
