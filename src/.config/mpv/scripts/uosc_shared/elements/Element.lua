...
function Element:reset_proximity()
  self.proximity, self.proximity_raw = 0, infinity
end
...
function Element:get_visibility()
  ...
  if not self.ignores_menu and Menu and Menu:is_open() then
    return 0
  end
...
function Element:tween(from, to, setter, factor_or_callback, callback)
  ...
  self._kill_tween = self.enabled
    and tween(from, to, setter, factor_or_callback, function()
...
function Element:tween_property(prop, from, to, factor_or_callback, callback)
  self:tween(from, to, function(value)
    self[prop] = value
  end, factor_or_callback, callback)
end
