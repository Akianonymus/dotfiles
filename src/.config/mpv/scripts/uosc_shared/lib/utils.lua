...
function tween(from, to, setter, factor_or_callback, callback)
  ...
  if type(factor_or_callback) == "function" then
    callback = factor_or_callback
  end
...
function normalize_path(path)
  ...
  if state.os == "windows" or is_unc then
    path = path:gsub("/", "\\")
  end
...
function decide_navigation_in_list(list, current_index, delta)
  ...
  if state.shuffle then
    ...
    while current_index == new_index do
      new_index = math.random(#list)
    end
...
function serialize_chapter_ranges(normalized_chapters)
  ...
  for i, normalized in ipairs(normalized_chapters) do
    chapters[i] = table_shallow_copy(normalized)
  end
