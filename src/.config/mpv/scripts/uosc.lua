...
function itable_slice(haystack, start_pos, end_pos)
  ...
  if end_pos < 0 then
    end_pos = #haystack + end_pos + 1
  end
  if start_pos < 0 then
    start_pos = #haystack + start_pos + 1
  end
...
local word_order_comparator = (function()
  ...
  return function(a, b)
    ...
    for i = 1, math.max(#a, #b) do
      local ai = a:sub(i, i)
      local bi = b:sub(i, i)
      if ai == nil and bi then
        return true
      end
      if bi == nil and ai then
        return false
      end
...
function tween_element_property(element, prop, from, to, speed, callback)
  tween_element(element, from, to, function(_, value)
    element[prop] = value
  end, speed, callback)
end
...
function utf8_iter(string)
  ...
  return function()
    ...
    byte_count = 1
    if char_byte < 192 then
      byte_count = 1
    elseif char_byte < 224 then
      byte_count = 2
    elseif char_byte < 240 then
      byte_count = 3
    elseif char_byte < 248 then
      byte_count = 4
    elseif char_byte < 252 then
      byte_count = 5
    elseif char_byte < 254 then
      byte_count = 6
    end
  ...
  for char_start, count, char_width in utf8_iter(text) do
    ...
    if can_wrap or (char_end == #text) then
      ...
      if line_width_after_remove < line_width_requested then
        ...
        if (line_width_requested - before_width) < (line_width_after_remove - line_width_requested) then
          ...
          if before_width > max_width then
            max_width = before_width
          end
        else
    ...
    if current_file.basename == file then
      if direction == "forward" then
        if files[index + 1] then
          return utils.join_path(current_file.dirname, files[index + 1])
        end
...
function Element.new(props)
  ...
  element._flash_out_timer = mp.add_timeout(options.flash_duration / 1000, function()
    local getTo = function()
      return element.proximity
    end
...
function Element:trigger(name, ...)
  ...
  for _, handler in ipairs(self._eventListeners[name]) do
    handler(...)
  end
  request_render()
end
...
function Menu:open(items, open_item, opts)
  ...
  elements:add(
    ...
    Element.new({
      ...
      open_selected_item_soft = function(this)
        this:open_selected_item(true)
      end,
      close = function(this)
        menu:close()
      end,
...
function Menu:disable_key_bindings()
  for _, name in ipairs(menu.key_bindings) do
...
function Menu:create_action(name)
  return function(...)
    if elements.menu then
      elements.menu:maybe(name, ...)
    end
  end
end
...
function update_element_cursor_proximity(element)
  if cursor.hidden then
    ...
    element.proximity = menu:is_open() and 0
      or 1 - (math.min(math.max(element.proximity_raw - options.proximity_in, 0), range) / range)
  end
end
...
function update_proximities()
  ...
  for _, element in elements:ipairs() do
    ...
    if element.on_global_mbtn_left_down then
      capture_mbtn_left = true
    end
    if element.on_global_wheel_up or element.on_global_wheel_down then
      capture_wheel = true
    end

    if element.proximity_raw == 0 then
      -- Element has local forced key listeners
      if element.on_mbtn_left_down then
        capture_mbtn_left = true
      end
      if element.on_wheel_up or element.on_wheel_up then
        capture_wheel = true
      end
...
function render_timeline(this)
  ...
  local function render_chapters()
    if
      ...
      or ((state.chapters == nil or #state.chapters == 0) and state.ab_loop_a == nil and state.ab_loop_b == nil)
    then
      return
    end
  ...
  if (this.proximity_raw == 0 or this.pressed) and not (elements.speed and elements.speed.dragging) then
    ...
    ass:new_event()
    ass:append(
      "{\\blur0\\bord0\\xshad-1\\yshad0\\1c&H"
...
function render_top_bar(this)
  ...
  if not this.enabled or opacity == 0 then
    return
  end
  ...
  if options.top_bar_title and state.media_title then
    ...
    ass:new_event()
    ass:append(
      "{\\q2\\blur0\\bord1\\shad0\\1c&HFFFFFF\\3c&H000000\\fn"
        .. config.font
        .. "\\fs"
        .. this.font_size
        .. bold_tag
...
function render_volume(this)
  ...
  if this.width == 0 or opacity == 0 or not state.has_audio then
    return
  end
  ...
  if slider.height > 0 then
    ...
    local fay = slider.ay
      + (height_without_border * (1 - math.min(state.volume / state.volume_max, 1)))
    ...
    ass:new_event()
    ass:append(
      "{\\blur0\\bord0\\1c&H" .. options.color_background .. "\\iclip(" .. fpath.scale .. ", " .. fpath.text .. ")}"
    )
  ...
  ass:append(icon(
    icon_name,
    mute.ax + (mute.width / 2),
    mute.ay + (mute.height / 2),
    mute.width * 0.7, -- x, y, size
    0,
    0,
    options.volume_border, -- shadow_x, shadow_y, shadow_size
    "background",
    options.volume_opacity * opacity -- backdrop, opacity
  ))
  return ass
end

function render_speed(this)
  if not this.dragging and (elements.curtain.opacity > 0) then
    return
  end
...
function render_menu_button(this)
  ...
  if this.width == 0 or opacity == 0 then
    return
  end
  ...
  ass:append(icon(
    "menu_button",
    burger.ax + (burger.width / 2),
    burger.ay + (burger.height / 2),
    burger.width, -- x, y, size
    0,
    0,
    options.menu_button_border, -- shadow_x, shadow_y, shadow_size
    "background",
    options.menu_button_opacity * opacity -- backdrop, opacity
  ))
  return ass
end
    ...
    if item_by >= this.ay and item_ay <= this.by then
      ...
      elseif has_submenu then
        ...
        ass:append(icon(
          ...
          0,
          0,
          1, -- shadow_x, shadow_y, shadow_size
          is_active and "foreground" or "background",
          this.opacity, -- backdrop, opacity
          item_clip
        ))
      end
    end
...
elements:add(
  ...
  Element.new({
    ...
    update_size = function(this)
      this.size = options.window_border_size > 0
          and not state.fullormaxed
          and not state.border
          and options.window_border_size
        or 0
    end,
...
elements:add(
  ...
  Element.new({
    ...
    flash = function(this)
      if not this.is_manual and this.type ~= "flash" then
        return
      end
...
elements:add(
  ...
  Element.new({
    ...
    update_dimensions = function(this)
      ...
      this.font_size =
        math.floor(math.min((this.size_max + 60) * 0.2, this.size_max * 0.96) * options.timeline_font_scale)
...
if itable_find({ "left", "right" }, options.volume) then
  ...
  elements:add(
    ...
    Element.new({
      ...
      spacing = nil,
      on_display_change = function(this)
        if state.volume_max == nil or state.volume_max == 0 then
          return
        end
        this.ax = elements.volume.ax
      ...
      set_from_cursor = function(this)
        ...
        if state.volume ~= new_volume then
          mp.commandv("set", "volume", math.min(new_volume, state.volume_max))
        end
      end,
      on_mbtn_left_down = function(this)
        this.pressed = true
  ...
  elements:add(
    ...
    Element.new({
      ...
      on_mbtn_left_up = function(this)
        ...
        if
          this.dragging
          and math.abs(this.dragging.distance) < 6
          and mp.get_time() - this.dragging.start_time < 0.15
        then
...
function parse_chapters()
  ...
  if not chapters or not state.duration then
    return
  end
    ...
    for _, track in ipairs(tracklist) do
      if track.type == track_type then
        ...
        local hint_vals = {
          ...
          track["demux-h"]
            and (track["demux-w"] and track["demux-w"] .. "x" .. track["demux-h"] or track["demux-h"] .. "p"),
...
options.timeline_chapters = itable_find({ "dots", "lines", "lines-top", "lines-bottom" }, options.timeline_chapters)
...
options.timeline_cached_ranges = (function()
  if options.timeline_cached_ranges == "" or options.timeline_cached_ranges == "no" then
    return nil
  end
...
mp.add_key_binding(nil, "flash-volume", function()
  if elements.volume then
    elements.volume:flash()
  end
end)
mp.add_key_binding(nil, "flash-speed", function()
  if elements.speed then
    elements.speed:flash()
  end
end)
...
mp.add_key_binding(nil, "first-file", function()
  load_file_in_current_directory(1)
end)
mp.add_key_binding(nil, "last-file", function()
  load_file_in_current_directory(-1)
end)
