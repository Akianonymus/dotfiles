--https://raw.githubusercontent.com/Ashyni/mpv-scripts/master/dynamic-crop.lua
--[[
...
-- computed options
local function convert_sec_to_ms(num)
  return math.floor(num * 1000)
end
local function convert_ms_to_sec(num)
  return num / 1000
end
...
local function manage_filter(action, filter)
  return mp.command(string.format("no-osd vf %s @%s", action, filter))
end

local function insert_cropdetect_filter()
  if toggled > 1 or paused then
    return
  end
  -- "vf pre" cropdetect / "vf append" crop, in a proper order
  ::insert::
  local insert_filter = manage_filter(
    "pre",
    string.format(
      "%s:lavfi-cropdetect=limit=%d/255:round=%d:reset=%d%s",
      labels.cropdetect,
      limit.current,
...
local function compute_metadata(meta)
  ...
  if
    not meta.is_invalid and math.abs(meta.w - source.w) <= options.detect_round * 2
    or math.abs(meta.h - source.h) <= options.detect_round * 2
  then
    for ratio in string.gmatch(options.ratios, "%S+%s?") do
      for a, b in string.gmatch(ratio, "(%d+)/(%d+)") do
        ratio = a / b
      end
      local height = math.floor((meta.w * 1 / ratio) + 0.5)
...
local function print_debug(meta, type_, label)
  ...
  if type_ == "stats" and stats and stats.trusted then
    ...
    mp.msg.info(string.format("Limit - min/max: %s/%s | counter: %s", limit.min, options.detect_limit, limit.counter))
    mp.msg.info(
      string.format(
        "Trusted - unique: %s | offset: X:%sY:%s",
        stats.trusted_unique,
        read_maj_offset.x,
        read_maj_offset.y
      )
    )
    for whxy, ref in pairs(stats.trusted) do
      if stats.trusted[whxy] then
        mp.msg.info(
          string.format(
            "\\ %-29s | offX=%3s offY=%3s | applied=%s overall=%s last_seen=%s",
            whxy,
            ref.offset.x,
            ref.offset.y,
            ref.applied,
            convert_ms_to_sec(ref.time.overall),
    ...
    mp.msg.info(
      "Buffer - unique: " .. buffer.unique_meta .. " | total: " .. buffer.index_total,
      convert_ms_to_sec(buffer.time_total) .. "sec | known_ratio:",
...
local function process_metadata(timestamp, collected)
  ...
  if buffer.index_known_ratio > 0 then
    buffer.time_known = buffer.time_known + elapsed_time
  end

  -- reset last_seen before correction
  if stats.trusted[collected.whxy] and collected.time.last_seen < 0 then
    collected.time.last_seen = 0
  end
  ...
  local new_ready = not stats.trusted[collected.whxy]
    and collected.is_trusted_offsets
    and not collected.is_invalid
    and (
      collected.is_known_ratio and collected.time.buffer >= new_known_ratio_timer
      or fallback and not collected.is_known_ratio and collected.time.buffer >= new_fallback_timer
    )
  if new_ready then
  ...
  if not current.is_invalid and not current.is_trusted_offsets then
    ...
    for _, ref in pairs(stats.trusted) do
      ...
      table.sort(diff.closest_side, function(k1, k2)
        return k1 < k2
      end)
      ...
      if closest.ref and diff.vs_current >= 1 and diff.vs_current == closest.vs_current then
        for i = 1, diff.vs_current do
          if diff.closest_side[i] < closest.closest_side[i] then
            closest_side = true
          end
          if diff.closest_side[i] ~= closest.closest_side[i] then
            break
  ...
  if options.detect_round <= 4 and stats.trusted[current.whxy] then
    ...
    local applied_in_margin = math.abs(current.w - applied.w) <= margin and math.abs(current.h - applied.h) <= margin
    for _, ref in pairs(stats.trusted) do
      ...
      if in_margin then
        ...
        local pattern = not applied_in_margin and ref.time.overall > current.time.overall or gt_applied or applied_gt
  ...
  for whxy, ref in pairs(stats.trusted) do
    if whxy ~= current.whxy then
      if ref.time.last_seen > 0 then
        ref.time.last_seen = 0
      end
  ...
  -- apply crop
  local detect_source = current == last_current
    and (current.is_source or collected.is_source and not limit.target == -1)
  local confirmation = not current.is_source
    and stats.trusted[current.whxy]
    and current.time.last_seen >= fast_change_timer
    and (not corrected.ref or current == last_current)
  ...
  if crop_filter and (not timestamps.prevent or timestamp >= timestamps.prevent) then
    ...
    if prevent_change_timer > 0 then
      ...
      if
        options.prevent_change_mode == 1 and (current.w > applied.w or current.h > applied.h)
        or options.prevent_change_mode == 2 and (current.w < applied.w or current.h < applied.h)
        or options.prevent_change_mode == 0
      then
  ...
  -- cleanup buffer
  local function time_to_cleanup_buffer(time_1, time_2)
    return time_1 > time_2 * (1 + options.segmentation)
  end
  ...
  if not fallback then
    buffer_timer = new_known_ratio_timer
  end
  local function proactive_cleanup() -- start to cleanup if too much unique meta are present
    return buffer.time_total > buffer.time_known
      and buffer.unique_meta
        > buffer.index_total * (buffer_timer * options.segmentation / (buffer_timer * (1 + options.segmentation))) + 1
  end
  ...
  if
    timestamp < limit.timer
    and (collected.is_source and limit.target >= 0 or not collected.is_source and stats.trusted[collected.whxy])
  then
    limit.timer = timestamp -- reset
  end
  if
    not collected.is_source
    and not current.is_invalid -- stable limit
    and (
      collected.is_trusted_offsets
      or collected == last_collected
      or current == last_current and corrected.ref and not current.is_source
    )
  then
    limit.target = 0
  ...
  elseif
    ...
    or current.is_source
      and corrected.ref
      and (corrected.vs_current == 1 or not applied.is_source and corrected.diff_3_4)
  then -- increase limit
    limit.target = 1
...
local function update_time_pos(_, timestamp)
  ...
  if not timestamps.insert then
    timestamps.insert = timestamps.current
  end

  if
    in_progress
    or not collected_.whxy
    or not timestamps.previous
    or filter_inserted
    or seeking
    or paused
    or toggled > 1
    or timestamp < options.start_delay
  then
    return
  end
...
local function playback_events(event, id, error)
  if event["event"] == "seek" then
    seeking = true
    if not paused then
      seek(event["event"], false)
    end
  else
    if not paused then
      resume(event["event"], false)
    end
    seeking = false
  end
end
